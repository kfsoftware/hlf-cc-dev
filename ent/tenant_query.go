...
func (tq *TenantQuery) QueryChaincodes() *ChaincodeQuery {
	query := &ChaincodeQuery{config: tq.config}
...
func (tq *TenantQuery) Clone() *TenantQuery {
	...
	return &TenantQuery{
		...
		withChaincodes: tq.withChaincodes.Clone(),
...
// WithChaincodes tells the query-builder to eager-load the nodes that are connected to
// the "chaincodes" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TenantQuery) WithChaincodes(opts ...func(*ChaincodeQuery)) *TenantQuery {
	query := &ChaincodeQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withChaincodes = query
	return tq
}
...
func (tq *TenantQuery) sqlAll(ctx context.Context) ([]*Tenant, error) {
	...
	if query := tq.withChaincodes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Tenant)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Chaincodes = []*Chaincode{}
		}
		query.withFKs = true
		query.Where(predicate.Chaincode(func(s *sql.Selector) {
			s.Where(sql.InValues(tenant.ChaincodesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.tenant_chaincodes
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "tenant_chaincodes" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "tenant_chaincodes" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Chaincodes = append(node.Edges.Chaincodes, n)
		}
	}
