...
// The ChaincodeFunc type is an adapter to allow the use of ordinary
// function as Chaincode mutator.
type ChaincodeFunc func(context.Context, *ent.ChaincodeMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ChaincodeFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ChaincodeMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ChaincodeMutation", m)
	}
	return f(ctx, mv)
}
